...
impl Node for RouteMatchNode {
    fn execute<'a>(
        ...
        Box::pin(async move {
            let expected_method = ctx.node_data.get("method")
                .and_then(|v| v.as_str())
                .unwrap_or("GET")
                .to_uppercase();
            let expected_path = ctx.node_data.get("path")
                .and_then(|v| v.as_str())
                .unwrap_or("/");

            let actual_method = ctx.resolve_input("in:method").await
                .and_then(|v| v.as_str().map(|s| s.to_uppercase()))
                .unwrap_or_default();
            let actual_path = ctx.resolve_input("in:path").await
            ...
            let path_match = expected_path == "*"
                ...
                || (expected_path.ends_with("/*") && actual_path.starts_with(&expected_path[..expected_path.len()-1]));
